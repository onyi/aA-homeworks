...
def top_titles
  ...
  Movie.select('id, title').where('score >= 9').as_json
end
...
def below_average_years
  ...
  # hint: use 'select', 'where', 'group', 'order'
    Movie.select('yr, COUNT(title) as bad_movies').where('score < 5').group('yr').order('bad_movies desc').as_json
end
...
def alphabetized_actors
  ...
  Actor.select('id, name').order('name asc').limit('10').as_json
end
...
def pulp_fiction_actors
  ...
  Movie.select('actors.id, actors.name').joins(:actors).where('title = \'Pulp Fiction\'').as_json
end
