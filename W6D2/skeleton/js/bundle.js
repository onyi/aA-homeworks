...
View.prototype.exercise4 = function () {
  ...
  //your code here!
  const $strong = $(`<strong>Onyi</strong>`);
  $('li.square').each( function(idx){
    if (idx % 2 == 0){
      $(this).text("OC");
    }
...
View.prototype.exercise5 = function () {
  ...
  $('li.square').each(function (idx) {
      $(this).on("click", event => {
        alert(`You clicked on position: ${$(this).attr("data-pos")}`);
      })
...
View.prototype.exercise7 = function(){
  ...
  $('li.square').each(function () {
    $(this).on("mouseover", event => {
      console.log(`Color: ${$(this).attr('style').split(';').filter( item => item.includes('background-color'))[0].split(":")[1]}`);
    })
  });
};
